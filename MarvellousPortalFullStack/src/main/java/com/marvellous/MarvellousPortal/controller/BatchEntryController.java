...
public class BatchEntryController
{
    ...
    @PutMapping("/id/{myid}")
    public ResponseEntity<?> updateEntryById(@PathVariable ObjectId myid,@RequestBody BatchEntry batchEntry )
    {
        BatchEntry old = batchEntryService.findById((myid)).orElse(null);
        if(old != null)
        {
            old.setName(batchEntry.getName());
            old.setFees(batchEntry.getFees());
